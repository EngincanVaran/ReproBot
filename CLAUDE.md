...
- `state.py` — `ReproState`, the project plan's §1.2 shared-memory object implemented faithfully. `critic_output` is present and permanently `None` so the schema doesn't change when the Critic lands. `history` is an append-only audit log of structured entries (timestamp/stage/event/detail), which is what lets the loop answer "have we already tried this".
- `loop.py` — the retry loop as three **pure** decision functions (`decide_after_run`, `decide_after_syntax_error`, `decide_after_regeneration`) plus a thin driver. Keeping the routing pure is why all seven terminal verdicts are unit-testable with no Docker and no API key.
...
- `pipeline.py` — the only entry point, same CLI shape as the other stages, plus `--retry-budget`, `--plateau-threshold`, and `--use-existing-script` (seeds attempt 1 from a script already on disk instead of paying to regenerate it).
- **Deliberately NOT LangGraph**, despite the project plan's §1.3 recommendation: at this slice the graph is three nodes and one conditional edge. The state object is exactly §1.2-shaped and the routing is isolated in pure functions, so migrating is mechanical once the Critic adds real branching. Revisit when that lands, not before.
- **No Critic.** Nothing compares a reproduced number to the paper's claim. That's correct for now — a `probe` run is two optimizer steps, so its numbers are noise, and comparing them to a paper's headline result would be meaningless.
