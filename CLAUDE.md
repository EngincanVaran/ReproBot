...
Implementation has started, narrowly: **`ocr/` (PDF extraction), `reader/` (claims, hyperparameters, data_pipeline extraction, plus a validation retry loop), and `coder/` (training-script generation) are the pipeline stages with real code.** Everything else (Reader's architecture-notes extraction, Runner, Critic, Orchestrator) is still design-only, described in `docs/project-plan/ReproBot_Project_Plan.md`. `runner/` is scoped in detail but not built — see `docs/agent-log.md`'s Research Agent entry for the Docker/dry-run design, and note its blocking prerequisite (the Docker daemon must be running). Alongside `ocr/`/`reader/`, the rest of the repo is still the project proposal, progress reports, and a literature review built from 9 related papers, organized under `docs/`.
