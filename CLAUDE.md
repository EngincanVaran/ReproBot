...
**`runner/`'s Docker path is written but unproven** — the daemon was blocked behind a macOS privileged-access prompt when it was built, so no image has ever been built and no container has ever run. Everything daemon-independent *is* verified (see `runner/README.md`'s Status section). Treat "the image builds" as an open question, not an assumption. Alongside `ocr/`/`reader/`, the rest of the repo is still the project proposal, progress reports, and a literature review built from 9 related papers, organized under `docs/`.
...
### runner/ — Docker-sandboxed execution (written, Docker path unproven)
