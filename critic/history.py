...
def main() -> None:
    ...
    paper_dir = args.coder_output / args.paper
    metrics_path = paper_dir / f"metrics.{args.mode}.json"
    if not metrics_path.exists():
        metrics_path = paper_dir / "metrics.json"
    log_path = args.runner_output / args.paper / "logs" / f"{args.mode}.stdout.log"
    for path in (metrics_path, log_path):
        if not path.exists():
            parser.error(f"missing {path}")

    metrics = json.loads(metrics_path.read_text(encoding="utf-8"))
    reader_path = args.reader_output / f"{args.paper}.json"
    target = None
    ...
    if not evals:
        parser.error(f"no per-epoch evaluation lines found in {log_path}")

    records = build_records(evals, losses, metrics, target=target, chance=args.chance)
    out = paper_dir / f"metrics.{args.mode}.history.jsonl"
