"""Generate the ReproBot second progress report in both column formats.

The body is held once, so the two .tex files are guaranteed to carry identical
prose - the first report's pair differ only in documentclass and geometry, and
this keeps that property by construction rather than by careful copy-editing.
Wide figures become figure* in the two-column build; FIGSTAR is the marker.
"""
...
out = Path("docs/progress-reports/second-progress-report")
out.mkdir(parents=True, exist_ok=True)
(out / "second_report_singlecolumn.tex").write_text(build(False), encoding="utf-8")
(out / "second_report_twocolumn.tex").write_text(build(True), encoding="utf-8")
